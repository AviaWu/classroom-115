...
| `operations/{operationId}` | 物件 | 一筆已提交操作的固定收據，以操作 ID 為鍵。 |
...
| `bossProgress` | BOSS 進度陣列 | 每隻 BOSS 的個人剩餘血量、密碼驗證、答對題目與擊敗狀態。 |
...
一般五秒輪詢只讀 `progress`，因此不下載圖片。畫板開啟後才讀缺少的三張圖片；同一 ID 在該次頁面工作階段會使用記憶體快取。老師後台沒有畫冊或歷史畫作。
...
前景且連線中的頁面每 **5 秒只讀取 `progress`**，回到頁面或恢復連線時也會重新確認。只有待確認操作需要另外查詢其收據；一般輪詢不下載整個操作收據集合。

實際操作在 `/games/classroom-115` 班級根節點進行 REST 條件式交易：先讀取最新班級資料及 ETag，在最新進度上計算操作，再以 `If-Match` 條件寫入新進度、操作收據與最新操作 ID。若其他裝置先寫入，Firebase 回傳 `412`，程式重新讀取再計算。ETag 只供傳輸層判斷競爭，不存入 `progress`。
