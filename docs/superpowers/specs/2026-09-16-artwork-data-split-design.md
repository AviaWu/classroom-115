...
一般畫面更新只根據畫作索引判斷最新作品，不能因為圖片尚未載入而刪除索引或回寫空資料。進度正規化器接受 `{id, savedAt}`，不再要求 `data`，並移除舊的 `drawingAlbum` 欄位。
...
還原既有備份時，合併舊格式的 `drawings` 與 `drawingAlbum`，依有效 `savedAt` 由新到舊排序，只取最新三張。先將這三張的 `data` 寫入獨立作品節點，再以不含 `data` 的索引還原 `progress`。其餘歷史畫作不還原。所有保留圖片寫入成功後才提交進度還原操作，重試沿用備份中的畫作 ID。

部署新版後，第一次讀到仍在 `progress` 內含 `data` 或 `drawingAlbum` 的舊資料時，只在記憶體辨識為待遷移，不因開頁自動寫入。老師後台「備份」頁提供明確的「遷移畫作資料」操作：

1. 從既有 `drawings` 與 `drawingAlbum` 選出最新三張。
2. 將這三張寫入獨立作品節點。
3. 全部寫入成功後，以單一班級交易把 `progress.drawings` 改成索引並移除 `drawingAlbum`。
4. 不屬於最新三張的既有獨立圖片 ID 加入待清理清單。

全部保留作品寫入成功前不移除舊 `data`，所以中途失敗可安全重試。遷移確認畫面明確告知歷史畫冊會永久刪除，只保留最新三張。
...
- 一般進度規則要求 `drawings` 最多三筆，每筆只能保存 `id`、`savedAt`，不得保存 `data`。
- `drawingAlbum` 不允許存在。
...
- 備份與還原測試確認只輸出及還原最新三張、缺圖不產生不完整備份、舊格式畫冊只選最新三張。
- 遷移測試確認開頁不寫入、部分失敗不刪除舊資料、成功後移除 `data` 與 `drawingAlbum`，並排入歷史圖片清理。
