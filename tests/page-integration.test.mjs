...
test('boss battle shows its image and HP, advances automatically, and hides a defeated boss',async t=>{
    ...
    await h.run("answerBossQuestion(1,'dragon','q2',1,'')");assert.equal(h.cloud.students[0].bossProgress[0].hp,6);assert.match(h.w.document.querySelector('.boss-battle-message').textContent,/回復 5 點血量/);assert.equal(h.w.document.querySelector('[aria-label="巨龍戰鬥血量"]').getAttribute('aria-valuenow'),'6');
