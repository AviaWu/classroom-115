...
test('backend synchronizes the new built-in clothing and pet products to cloud',async t=>{
    ...
    const boy=h.cloud.clothesM.find(item=>item.id==='builtin_boy_event_ba9');
    const girl=h.cloud.clothesF.find(item=>item.id==='builtin_girl_event_ga6');
    const pet=h.cloud.layouts.find(item=>item.id==='builtin_pet_event_decb_18');
    assert.deepEqual(boy,{id:'builtin_boy_event_ba9',name:'男生活動服裝 9',level:'活動贈送',price:0,active:true,image:'/images/boy/ba9.png'});
    assert.deepEqual(girl,{id:'builtin_girl_event_ga6',name:'女生活動服裝 6',level:'活動贈送',price:0,active:true,image:'/images/girl/ga6.png'});
    assert.deepEqual(pet,{id:'builtin_pet_event_decb_18',name:'十二月活動寵物 18',level:'活動贈送',price:0,active:true,image:'/images/Dec2/decb%20(18).png'});
...
test('boss battle shows its image and HP, advances automatically, and hides a defeated boss',async t=>{
    ...
    assert.match(h.w.document.querySelector('.pet-level-card').textContent,/攻擊力 7/);assert.equal(h.w.document.querySelectorAll('.boss-card').length,2);assert.equal(h.w.document.querySelector('[aria-label="巨龍血量"]').getAttribute('aria-valuenow'),'8');
    h.w.Math.random=()=>0;h.setPrompt('1234');await h.run("startBossBattle(1,'dragon')");
    const shuffledButtons=h.w.document.querySelectorAll('#bossBattleArea .boss-options button');assert.equal(shuffledButtons.length,4);assert.equal(shuffledButtons[0].textContent,'A．2');assert.match(shuffledButtons[0].getAttribute('onclick'),/,1,'1234'\)$/);assert.match(h.w.document.querySelector('#bossBattleArea img').src,/boss%20\(1\)\.png/);assert.equal(h.w.document.querySelector('[aria-label="巨龍戰鬥血量"]').getAttribute('aria-valuenow'),'8');
    ...
    await h.run("answerBossQuestion(1,'dragon','q2',1,'')");assert.equal(h.cloud.students[0].bossProgress[0].hp,6);assert.match(h.w.document.querySelector('.boss-battle-message').textContent,/答錯了，BOSS加五點HP。/);assert.equal(h.w.document.querySelector('[aria-label="巨龍戰鬥血量"]').getAttribute('aria-valuenow'),'6');
