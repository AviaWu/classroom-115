...
export function credentialsForLogin(account,pin){
    ...
    if(!Number.isInteger(studentId) || studentId<1 || studentId>30) return null;
...
export function accountForEmail(email){
    ...
    const studentId=Number(match?.[1]);
    return Number.isInteger(studentId) && studentId>=1 && studentId<=30 ? {role:'student',studentId} : null;
}
